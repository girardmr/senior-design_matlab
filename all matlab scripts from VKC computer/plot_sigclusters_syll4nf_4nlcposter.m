%plot significant clusters in evoked data -syllables
% still need to
...
%of plots
clear all; clc
clustercode = 'Syll4nf_4nlcposter';
cond1= 'SSreg';
cond2= 'SWreg';
band = 'evoLoGamma';

load SSregvsSWreg_stat_logam_evo_rs_f.mat

load Strong_Strong_gravg_mblc_evo_rs.mat
bin1 = TFgravg_evo;
clear TFgravg_evo

load Strong_Weak_gravg_mblc_evo_rs.mat
bin2 = TFgravg_evo;
clear TFgravg_evo
...
latsnaps = [150 200]; % want snapshots at these latencies for this cluster in ms
zscale = [-0.75 0.75]; % %
...
% now define time limits straight from the stats
 % actual time limits of cluster for box in seconds
%
...
for k = 1:2;  %length(j)-1 subplots
    ...
    cfg.zlim = zscale;
   cfg.highlight = 'on';
    cfg.highlightchannel = find(C_mask(:,m(k))==1);       cfg.baseline = 'no';
...
%save SigChan_Syll4nf.mat SigChan

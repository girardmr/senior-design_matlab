...
load hdrtemplate_dap3_long.mat %this header template contains sampling rate
%channel labels for adult net (all channels) average ref, tmin (-1250ms) and tmax (+1246ms) for the segments
%time in seconds now % CAREFUL WRONG HEADER FILE USED HERE!!!
...
for m=1:length(S)
    subj=S{m};
    matfilename=cat(2,subj,'.newfil.seg.ref.mat') % ref files contain single trials (only good segments post-AD) 

%calculate power with wavelets on trial data (EVOKED) - Metronome beats
%took 1 min  to complete for all 16 subjects
%rlg october 18, 2010
...
for m=1:length(S) %for each subject
    ...
    for b=1:length(bin) %for each condition specified above
        filename= cat(2,'suj',suj,'_',bin{b},'_erp.mat')
        load(filename)
        cfg = [];
        ...
        TFRwave_evo = freqanalysis(cfg, data);

        outfile= cat(2,'suj',suj,'_',bin{b},'_tfr_evo.mat')
        save(outfile,'TFRwave_evo');
        clear filename data outfile TFRwave_evo
    end
    clear filename
end

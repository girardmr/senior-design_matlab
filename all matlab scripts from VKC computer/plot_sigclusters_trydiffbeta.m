...
%% THIS SHOULD HAVE BEEN RUN ON BETA 13-24Hz but was instead 13-20. MAYBE TRY RUNNING ON BETA WITHOUT avg over freq? 
clear all; clc
%this one goes from 
band = 'IndBeta';  %for naming purposes, say ERP here or the name of your frequency band of interest
clustercode = cat(2,'CDTwords_camp_',band); % give some abbreviation for your cluster, which will be used to name the graphics files
...
load(cat(2,'CDT_campers_',cond1, '_gravg_avblc_ind.mat'));
% use grand average files
bin1 = TFgravg_ind;
clear TFgravg_ind

load(cat(2,'CDT_campers_',cond2, '_gravg_avblc_ind.mat'));
bin2 = TFgravg_ind;
clear TFgravg_ind
...
latsnaps = [80 120 180]; % want "snapshots" at these latencies for this cluster - in ms
zscale = [-0.25 0.25]; % determine iteratively
...
cfg.ylim         = [8 25]; % could also use instead cfg.ylim = freqband 

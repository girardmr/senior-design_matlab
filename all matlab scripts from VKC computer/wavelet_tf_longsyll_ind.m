%calculate power with wavelets on trial data (induced) - long syllables
%took about 85 min. to complete for all 16 subjects
%rlg 28 jan 2011
...
for m=1:length(S) %for each subject
    ...
    for b=1:length(bin) %for each condition specified above
        filename= cat(2,'suj',suj,'_',bin{b},'L.mat')
        load(filename)
        cfg = [];
        ...
        TFRwave_ind = freqanalysis(cfg, data);

        outfile= cat(2,'suj',suj,'_',bin{b},'_L_tfr_ind.mat')

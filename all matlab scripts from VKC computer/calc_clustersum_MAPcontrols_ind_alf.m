...
clear all; clc
load MAPcamp_1vs2_ind_alf_stat.mat % USE CAMPERS' CLUSTER WITH CONTROLS' DATA
% CAREFUL, POS CLUSTER!!
%% define subjects - controls
...
for m=1:length(S)
    suj=S{m};
    for b = 1:length(bin)
        filename = cat(2,suj,'_MAP_',bin{b},'_tfr_avblc_ind.mat');
        
        load(filename)
        data{m}.(bin{b}) = TFRwave_ind;
...
dlmwrite('MAP_controls_indalf_1vs2_diffsums.txt',K,'delimiter','\t','precision','%.2f');

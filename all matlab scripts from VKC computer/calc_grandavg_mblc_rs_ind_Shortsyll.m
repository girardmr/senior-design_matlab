% load in baseline-corrected wavelet TFR data (induced) for prime syllablces
...
for m=1:length(S)
    ...
    for b=1:length(bin)
        file_cond.(bin{b}){m}= cat(2,'suj',suj,'_',bin{b},'_S_ind_rs_mblc.mat');
    end
...
for b=1:length(bin)
    ...
    outfile     =  cat(2,bin{b}, '_S_gravg_mblc_ind_rs.mat')

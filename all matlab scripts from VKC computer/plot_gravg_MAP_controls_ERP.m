...
for b=1:length(bin) %for each condition specified above
    ...
    filename = cat(2,'MAP_controls_',bin{b}, '_gravg_ERP.mat'); %load filtered baseline corr ERPs
    load(filename)
...
%suplabel('MAP gravg controls','t')

outfilename = 'MAP gravg controls ERP - E6'
...
saveas(gcf, 'ERP gravg music, MAP controls', 'fig'); % save as a Matlab Figure file
saveas(gcf, 'ERP gravg music, MAP controls', 'tif'); % save as a tiff file
...
saveas(gcf, 'ERP gravg faces, MAP controls', 'fig'); % save as a Matlab Figure file
saveas(gcf, 'ERP gravg faces, MAP controls', 'tif'); % save as a tiff file

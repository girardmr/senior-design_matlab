% load in baseline-corrected wavelet TFR data (EVOKED) for baseline corr
% syllable data
% average together individual subjects (or just collect into one file) and plot all channels
% save files
% modified by rlg 1 feb 2011 for old short SYll EPOCHS with new norm
clear all; clc
%% define subjects
...
for m=1:length(S)
    ...
    for b=1:length(bin)
        file_cond.(bin{b}){m}= cat(2,'suj',suj,'_',bin{b},'_S_evo_rs_mblc.mat');
    end
...
for b=1:length(bin)
    ...
    TFallSubj_evo   =    freqgrandaverage(cfg,TFdata.(bin{b}){:});
    outfile         =    cat(2,bin{b}, '_S_allSubj_mblc_evo_rs.mat');

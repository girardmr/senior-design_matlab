...
condname{1}='Expected Word Rhythm';
condname{2}='Unexpected Word Rhythm';
...
for b=1:length(bin) %for each condition specified above
    ...
    filename = cat(2,'CDT_controls_',bin{b}, '_gravg_ERP.mat'); %load filtered baseline corr ERPs
    load(filename)
...
subplot(2,2,3)
ft_singleplotER(cfg,gravgdata.(bin{1}),gravgdata.(bin{2}))
legend(condname{1},condname{2},'Location','North')
...
%suplabel('CDT gravg controls','t')

outfilename = 'CDT gravg controls ERP - Pz'
...
saveas(gcf, 'ERP gravg music, CDT controls', 'fig'); % save as a Matlab Figure file
saveas(gcf, 'ERP gravg music, CDT controls', 'jpg'); % save as a jpgf file

...
clustercode = cat(2,'tut_Chords2vs3',band); % give some abbreviation for your cluster, which will be used to name the graphics files

load Chords_2vs3_ERP_stat.mat % make sure you load the right stat file
cond1= stat.cond1; % we added the two condition names to the stat structure during clustering
cond2= stat.cond2;

load(cat(2,'chords_ERP_',cond1, '_gravg.mat')); % use grand average files
bin1 = ERPgravg;
clear ERPgravg

load(cat(2,'chords_ERP_',cond2, '_gravg.mat'));
bin2 = ERPgravg;
clear ERPgravg
...
latsnaps = [240 300 340]; % want "snapshots" at these latencies for this cluster - in ms
...
cfg = [];
elec=read_sens('GSN128_positions_4clustering.sfp'); % electrodes to plot; excludes eye channels
...
cfg.zparam        = 'avg';
cfg.xlim         = [-0.100 0.650];%time in sec for the ERP plot
cfg.colorbar = 'EastOutside';
cfg.layout       =  layout;  %
cfg.colormap     = cmap;
cfg.ylim = [-2 2]; % not necessary to use same scale from topoplot in ERPs

...
for b=1:length(bin) %for each condition specified above
    ...
    filename = cat(2,'dynattiversen_',bin{b}, '_gravg_12sTLD_avblc_evo.mat'); %    BASELINE CORRECTED
    load(filename)
    data.(bin{b}) = TFgravg_evo;
    clear TFgravg_evo
...
for b=1:length(bin) %for each condition specified above
    ...
    outfilename = cat(2,'gravg',bin{b},' evoked multichan avblc')
...
outfilename = cat(2,'gravg',' evoked beta topo avblc')
...
outfilename = cat(2,'gravg',' evoked gamma topo avblc')

% Custom script to plot a significant cluster on grand-average tutorial induced alpha data
% mod. by RLG 13 jan 2011

clear all; clc
%this one goes from 
band = 'Alpha';  %for naming purposes, say ERP here or the name of your frequency band of interest
clustercode = cat(2,'tut_Chords2vs3_ind_',band); % give some abbreviation for your cluster, which will be used to name the graphics files

load Chords_2vs3_avblc_ind_alf_stat.mat % make sure you load the right stat file
cond1= stat.cond1; % we added the two condition names to the stat structure during clustering
cond2= stat.cond2;

load(cat(2,cond1, '_chords_gravg_avblc_ind.mat')); % use grand average files
bin1 = TFgravg_ind;
clear TFgravg_ind
...
cfg.ylim         = [7 25]; % could also use instead cfg.ylim = freqband 

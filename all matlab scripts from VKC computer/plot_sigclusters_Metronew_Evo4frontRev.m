...
%this one goes frrom
band = 'BetaGamma';
clustercode = cat(2,'Metronew_evo_4frontiers_',band);
% POS cluster, be careful
load Metronew_beatvsoff_beta_evo_stat.mat %make sure this is new stat file
cond1= stat.cond1;
cond2= stat.cond2;

load(cat(2,cond1, '_gravg_avblc_evo.mat'));
bin1 = TFgravg_evo;
clear TFgravg_evo

load(cat(2,cond2, '_gravg_avblc_evo.mat'));
bin2 = TFgravg_evo;
clear TFgravg_evo
...
%define latsnaps and zscale by hand this cluster goes from -100 to 300ms
latsnaps = [0]; % want snapshots at these latencies for this cluster in ms
zscale = [-2 2]; % %
%this one goes from
...
plot([x1 x1],[y2 y1],'-k','LineWidth',2); hold on %hold on here so I can add more
...
clear stat
load Metronew_beatvsoff_logam_evo_stat.mat %make sure this is new stat file
...
maptitle = cat(2,clustercode,'maps');
%TFtitle = (cat(2,clustercode,' ',cond1,'vs', cond2,' clusterTFR ',band));
%supertitle(TFtitle);
...
TFtitle = cat(2,clustercode,'TFRs');
%TFtitle = (cat(2,clustercode,' ',cond1,'vs', cond2,' clusterTFR ',band));
%supertitle(TFtitle);
saveas(gcf, TFtitle, 'fig');
saveas(gcf, TFtitle, 'epsc');

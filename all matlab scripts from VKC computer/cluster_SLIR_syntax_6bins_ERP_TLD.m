...
for b=1:length(bin)
    ...
    filename = cat(2,bin{b}, '_8TLD_revstim_allSubj.mat'); % only 10 Ss
    load(filename);
    data{b} = ERPallSubj;
    clear ERPallSubj
...
save SLIR_syntax_1vs2_8TLD_revstim_ERP_stat.mat stat
clear stat

%% 
[stat]    = ft_timelockstatistics(cfg, data{3}, data{4}); 
stat.cond1 = bin{3}; stat.cond2 = bin{4};%
save SLIR_syntax_3vs4_8TLD_revstim_ERP_stat.mat stat
clear stat

%%
[stat]    = ft_timelockstatistics(cfg, data{5}, data{6}); 
stat.cond1 = bin{5}; stat.cond2 = bin{6};%
save SLIR_syntax_corrvsviol_8TLD_revstim_ERP_stat.mat stat
clear stat

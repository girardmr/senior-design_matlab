% load in baseline-corrected TFR INDUCED (new) on metronew
...
for m=1:length(S)
    ...
    for b=1:length(bin)
        file_cond.(bin{b}){m}= cat(2,'suj',suj,'_',bin{b},'_tfr_avblc_ind.mat') 
    end
...
for b=1:length(bin)
    ...
    outfile     =  cat(2,bin{b}, '_allSubj_avblc_ind.mat');

...
for m=1:length(S)
    ...
    bcifilename = cat(2,'suj',suj,'.AllSyll.bci'); %
    ...
    %comment out if you want to use only a subset of the categories
    %present. here I don't want the irregulars, but I have defined bins already (above), so don't need to use
    ...
    markupfilename=cat(2,'suj',suj,'.LongSyll.seg.evt') % Markup tool can only do batch if these are named EEGfilename + .evt
    ...
    outfile= cat(2,'suj',suj,'_allsyll_trialinfo.mat') % also save .mat structure in case you need further matlab manipulations
    save(outfile,'data_info');
    
    clearvars -except S  m bin

...
for m=1:length(S) %for each subject
    ...
    for b=1:length(bin)
        load (cat(2,'suj',suj,'_',bin{b},'L.mat')) % Longsyll only trial EEG data (just need to know dim of trials)
        cond = data;
        clear data
        ...
        outfile= cat(2,'suj',suj,'_metro_rs_match',match{b},'_L_ERP.mat') % change name to reflect bin that you're matching to
        ...
        cfg.toi     = -0.300:0.002:0.300;    %time of interest 
        cfg.keeptrials = 'no'; %return individual trials or average (default = 'no')
        TFRwave_evo = ft_freqanalysis(cfg, data_rs_ERP);
        
        outfile= cat(2,'suj',suj,'_metro_evo_rs_',match{b},'_L.mat') % change name to reflect bin that you're matching to
        ...
        % evoked analysis
        filename= cat(2,'suj',suj,'_',bin{b},'_L_tfr_evo.mat'); % new LongSyll analysis
        load(filename)
        ...
        outfile= cat(2,'suj',suj,'_',bin{b},'_L_evo_rs_mblc.mat')
        save(outfile,'TFRwave_evo');
        ...
        outfile= cat(2,'suj',suj,'_metro_ind_rs_',match{b},'_L.mat') % change name to reflect bin that you're matching to
        ...
        filename= cat(2,'suj',suj,'_',bin{b},'_L_tfr_ind.mat') % new monosyll analysis; raw
        load(filename)
        ...
        outfile= cat(2,'suj',suj,'_',bin{b},'_L_ind_rs_mblc.mat')
        save(outfile,'TFRwave_ind');
    ...
    outfile=cat(2,'metrobase_sepch_L_indANDevo_rs_suj',suj,'.mat')

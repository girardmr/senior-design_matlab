...
clustercode = cat(2,'CDTword_ctrl',band); % give some abbreviation for your cluster, which will be used to name the graphics files

load CDTctrl_Words_ERP_stat.mat % make sure you load the right stat file
cond1= stat.cond1; % we added the two condition names to the stat structure during clustering
cond2= stat.cond2;

load(cat(2,'CDT_controls_',cond1,'_gravg_ERP')); % use grand average files
bin1 = ERPgravg;
clear ERPgravg

load(cat(2,'CDT_controls_',cond2,'_gravg_ERP'));;
bin2 = ERPgravg;
clear ERPgravg
...
latsnaps = [748 800 848]; % want "snapshots" at these latencies for this cluster - in ms
...
A= 0:4:900; % careful here: specify timepoints in ms that were used in clustering analysis. 

...
clustercode = cat(2,'syntax_corrvsviol_pos_',band); % give some abbreviation for your cluster, which will be used to name the graphics files

load SLIR_syntax_corrvsviol_10TLDSs_ERP_stat.mat % make sure you load the right stat file
cond1= stat.cond1; % we added the two condition names to the stat structure during clustering
cond2= stat.cond2;
condname{1} = 'Syntactically Correct';
condname{2} = 'Syntactic Violation';


load(cat(2,cond1, '_lpf_gravg_10SsTLD.mat')); % use grand average files
bin1 = ERPgravg;
clear ERPgravg

load(cat(2,cond2, '_lpf_gravg_10SsTLD.mat'));
bin2 = ERPgravg;
clear ERPgravg
...
Big_mat = (stat.negclusterslabelmat == 1); %find time,channel pairs where%electrode belongs to significant cluster (pos cluster#1)

%define latsnaps and zscale by hand. This cluster's latency is: 430 to
%900ms
latsnaps = [450 550 650 750 850]; % want "snapshots" at these latencies for this cluster - in ms
zscale = [-3 3]; % determine iteratively
...
A= 200:2:900; % careful here: specify timepoints in ms that were used in clustering analysis. 
...
cfg.paramter        = 'avg';
cfg.xlim         = [-0.100 0.900];%time in sec for the ERP plot
cfg.colorbar = 'EastOutside';
cfg.layout       =  layout;  %
cfg.colormap     = cmap;
cfg.ylim = [-3 2]; % not necessary to use same scale from topoplot in ERPs
...
time4ticks = 0:0.200:0.800 ; %% put ticks at these points in seconds
...
title('Grand Average ERPs to critical verbs')
ERPtitle = cat(2,clustercode,'clusterERP');

saveas(gcf, ERPtitle, 'fig'); % saves two versions - Matlab fig file and tiff
...
figure
cfg.interactive = 'yes';
cfg.ylim = [-4 4]; % not necessary to use same maxmin from topoplot in ERPs

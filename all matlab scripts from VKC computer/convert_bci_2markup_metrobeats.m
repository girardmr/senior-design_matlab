...
% USE THIS SCRIPT FOR MY CONVERSION FROM OLD TO NEW METRONOME CLICK SEGMENTATION!
...
%the category names in the newly segmented files
newbin{1} = 'metrobeats';
newbin{2} = 'offbeats';
...
for m=1:length(S)
    ...
    bcifilename = cat(2,'suj',suj,'.fil.metrbeat.bci'); %
    ...
    bin = unique(C{1,1})% detect (old category names) bins automatically by category names
    
    TableChan = {};
    TableSegStatus = {};
    
    for b=1:length(newbin)
        %temp_ind= find (ismember(C{1,1}, bin{b})==1); %find row indices of this particular bin
        %HERE MODIFY TO TAKE ONLY FIRST THREE
        x = size(C{1,1},1) %find number of lines (# of segments total)
        a = 1:x;
        fourthseg = 4:4:x;
        V = ismember(a,fourthseg); %find indices of all segments except 4th 
        temp_ind = find (V==0);
                
        data_info.(newbin{b}).starttime = C{1,2}(temp_ind);
        data_info.(newbin{b}).segperbin = C{1,3}(temp_ind);
        data_info.(newbin{b}).status = C{1,4}(temp_ind);
        
        % cycle through columns, which is all channels for each segment
        
        for seg = 1:length(data_info.(newbin{b}).segperbin)
            %status = data_info.(bin{b}).status(seg)
            % all segment status - good and bad
            markstatus = {};
            markstatus{1,1} = 'segment';
            
            if strcmp('good',data_info.(newbin{b}).status(seg)) == 1;
                markstatus{1,2} = 'good';
            elseif strcmp('bad',data_info.(newbin{b}).status(seg)) == 1;
                markstatus{1,2} = 'bad';
            end
            
            markstatus{1,3} = cat(2,'{',newbin{b},'}'); % category
            ...
            for n=5:2:(length(C)-1) %odd columns are channel #'s
                ...
                data_info.(newbin{b}).bciperseg{seg,1}(chan,1:2) = [chan bcinfo];
    ...
    markupfilename=cat(2,'MarkupBCI_metro_suj',suj,'.txt')
    ...
    outfile= cat(2,'subj',suj,'_metro_trialinfo.mat') % also save .mat structure in case you need further matlab manipulations
    save(outfile,'data_info');
    
    clearvars -except S  m newbin

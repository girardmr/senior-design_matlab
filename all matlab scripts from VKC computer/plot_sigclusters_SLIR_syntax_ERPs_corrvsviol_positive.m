...
load SLIR_syntax_corrvsviol_ERP_stat.mat % make sure you load the right stat file
cond1= stat.cond1; % we added the two condition names to the stat structure during clustering
cond2= stat.cond2;

load(cat(2,cond1, '_gravg.mat')); % use grand average files
bin1 = ERPgravg;
clear ERPgravg

load(cat(2,cond2, '_gravg.mat'));
bin2 = ERPgravg;
clear ERPgravg
...
latsnaps = [150 200 250 300 350]; % want "snapshots" at these latencies for this cluster - in ms
...
A= 0:2:1198; % careful here: specify timepoints in ms that were used in clustering analysis. 
...
cfg.paramter        = 'avg';
cfg.xlim         = [-0.100 1.200];%time in sec for the ERP plot
cfg.colorbar = 'EastOutside';
cfg.layout       =  layout;  %
cfg.colormap     = cmap;
...
time4ticks = 0:0.200:0.600:1.000; %% put ticks at these points in seconds

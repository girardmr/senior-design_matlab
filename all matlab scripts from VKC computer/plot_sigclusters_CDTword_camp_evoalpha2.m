...
% mod. by RLG april 27, 2012
%%  
clear all; clc
%this one goes from 
band = 'EvoAlpha2';  %for naming purposes, say ERP here or the name of your frequency band of interest
clustercode = cat(2,'CDTwords_camp_',band,'_4APSposter'); % give some abbreviation for your cluster, which will be used to name the graphics files
...
load(cat(2,'CDT_campers_',cond1, '_gravg_avblc_evo.mat'));
% use grand average files
bin1 = TFgravg_evo;
clear TFgravg_evo

load(cat(2,'CDT_campers_',cond2, '_gravg_avblc_evo.mat'));
bin2 = TFgravg_evo;
clear TFgravg_evo

%% step 2: fevo data at right times and frequencies to plot
...
% the first one.
% Big_mat = (stat.negclusterslabelmat == 1); %fevo time,channel pairs where%electrode belongs to significant cluster (negative cluster#1)
Big_mat = (stat.posclusterslabelmat == 2); %find time,channel pairs where%electrode belongs to significant cluster (pos cluster#2)
% SECOND cluster - late one
%define latsnaps and zscale by hand. This cluster's latency is
latsnaps = [500]; % want "snapshots" at these latencies for this cluster - in ms
...
A= 0:4:800; % careful here: specify timepoints in ms that were used in clustering analysis. 
...
cfg.ylim         = [7 15]; % could also use instead cfg.ylim = freqband 
cfg.xlim         = [0 0.800]; %time in seconds for TFR plot
cfg.colorbar = 'EastOutside';
cfg.layout       =  layout; 
cfg.colormap     = cmap;

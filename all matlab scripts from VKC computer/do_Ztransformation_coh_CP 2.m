...
for m=1:length(Spre) %for each subject
    ...
    for b=1:length(bin) %coh for pre vs post
        load(cat(2,'CP',sujPre,'_',bin{b},'_fft_coh.mat'))
        coh1 = FFTcoh;
        clear FFTcoh;
        
        load(cat(2,'CP',sujPost,'_',bin{b},'_fft_coh.mat'))
        coh2 = FFTcoh;
        clear FFTcoh;
        ...
        FFTcoh = coh1Z;
        outfile= cat(2,'CP',sujPre,'_',bin{b},'_fft_coh_Z.mat')
        save(outfile,'FFTcoh')
        clear FFTcoh outfile coh1Z
        
        coh2Z.wpli_debiasedspctrm = atanh(coh2.wpli_debiasedspctrm);
        FFTcoh = coh2Z;
        outfile= cat(2,'CP',sujPost,'_',bin{b},'_fft_coh_Z.mat')
        save(outfile,'FFTcoh')
        ...
        outfile= cat(2,'CP',sujPre,'vspost_',bin{b},'_fft_coh_Z.mat')
        save(outfile,'cohdif');
        clear outfile
        ...
        outfile= cat(2,'CP',sujPre(1:end-4),'_dummy_',bin{b},'_fft_coh_Z.mat')
